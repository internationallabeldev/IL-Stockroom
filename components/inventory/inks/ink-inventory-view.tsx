...
export function InkInventoryView({ initialLots, canManage, canRequest, inkCatalog }: Props) {
  ...
  const viewToggle = (
    <div id="ink-inv-view-toggle" className="flex border border-border shrink-0">
      <button
        onClick={() => switchView('table')}
        title={embedded ? 'Lista' : undefined}
        aria-label={embedded ? 'Lista' : undefined}
        className={cn(
      ...
      <button
        ...
        title={embedded ? 'Grupo' : undefined}
        aria-label={embedded ? 'Grupo' : undefined}
        className={cn(
      ...
      <div className={cn('sticky z-30 bg-background border-b border-border mb-6', toolbarStickyClass(embedded))}>
        ...
        <div className={cn(
          ...
          <div className={cn('flex items-center gap-2', embedded ? 'shrink-0' : 'flex-wrap @2xl:ml-auto')}>
            {embedded ? (
              <>
                <InkInventoryStatsBar lots={lots as InkLot[]} iconOnly />
