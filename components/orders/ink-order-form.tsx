...
export function InkOrderForm({ open, onClose, providers, inkCatalog, companyAddress }: Props) {
  ...
  const { fields, append, remove } = useFieldArray({ control, name: 'ink_items' })
  ...
  const inkProviders = providers.filter(
    p => p.provider_type === 'INK_SUPPLIER' || p.provider_type === 'BOTH'
  )

  const filtered = inkCatalog.filter(i =>
    !search ||
    i.name.toLowerCase().includes(search.toLowerCase()) ||
    i.code.toLowerCase().includes(search.toLowerCase())
  )

  const addedIds = fields.map(f => (f as any).ink_catalog_id)
          ...
          <div className="border border-border/50">
            ...
            <div className="px-4 py-3 space-y-3">
              ...
              <InkSelector
                ...
                onSelect={ink => {
                  append({ ink_catalog_id: ink.id, units_ordered: 1, kg_per_unit: 0, item_notes: null })
                  setSearch('')
                }}
              />
            </div>
