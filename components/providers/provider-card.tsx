...
export function ProviderCard({ provider, canEdit, onView, onEdit }: Props) {
  ...
  return (
    <div
      className={`bg-[#fdf9f0] border border-[#1A1A1A]/15 p-6 flex flex-col gap-4 transition-opacity ${!provider.enabled ? 'opacity-50' : ''
        }`}
    >
      {/* Header */}
        ...
        >
          {provider.logo_url ? (
            ...
            <img
              ...
              alt={provider.name}
              className="size-10 object-contain shrink-0 border border-[#1A1A1A]/10"
            />
          ) : (
            <div className="size-10 shrink-0 bg-[#E5E1D8] flex items-center justify-center border border-[#1A1A1A]/10">
              <span className="font-heading font-bold text-base text-[#1A1A1A]/40">
      ...
      {/* Contact */}
      <div className="space-y-1.5 border-t border-[#1A1A1A]/10 pt-4">
        <a
          href={`mailto:${provider.email}`}
          className="flex items-center gap-2 text-[11px] text-[#5f5e59] hover:text-[#1A1A1A] transition-colors"
        >
        ...
        {provider.whatsapp && (
          <a
            ...
            className="flex items-center gap-2 text-[11px] text-[#5f5e59] hover:text-[#25D366] transition-colors"
          >
      ...
      <p className="text-[10px] text-[#5f5e59]/70 truncate">{provider.address}</p>
