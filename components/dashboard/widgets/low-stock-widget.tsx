...
function StockRow({ item }: { item: LowStockItem }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-[#1A1A1A]/8 last:border-0">
      {item.color_code && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {crit && (
            <span className="shrink-0 text-[8px] font-bold uppercase tracking-widest text-[#ba1a1a] bg-[#ba1a1a]/10 px-1.5 py-0.5">
              Sin stock
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <div className="flex-1 h-1.5 bg-[#E5E1D8] overflow-hidden">
            <div
              className={cn('h-full transition-all', crit ? 'bg-[#ba1a1a]' : warn ? 'bg-yellow-500' : 'bg-green-500')}
              style={{ width: `${pct}%` }}
            />
          </div>
          <span className={cn('text-[9px] font-mono shrink-0 w-16 text-right', crit ? 'text-[#ba1a1a]' : 'text-[#1A1A1A]/50')}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          {!isLoading && all.length > 0 && (
            <span className="text-[8px] font-bold uppercase tracking-widest text-[#ba1a1a] bg-[#ba1a1a]/10 px-1.5 py-0.5">
              {all.length}
            </span>
          )}
        </div>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-10 bg-[#E5E1D8] animate-pulse" />
          ))}
        </div>
      ) : all.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center py-8 gap-2">
          <span className="size-2 rounded-full bg-green-500" />
          <p className="text-[9px] font-bold uppercase tracking-widest text-[#1A1A1A]/40">Todo en stock</p>
        </div>
      ) : (
        <div className="max-h-80 overflow-y-auto no-scrollbar">
          {type === 'BOTH' && inks.length > 0 && (
            <p className="text-[8px] font-bold uppercase tracking-widest text-[#1A1A1A]/30 mb-2">Tintas</p>
          )}
          {inks.map(item => <StockRow key={`ink-${item.id}`} item={item} />)}

          {type === 'BOTH' && papers.length > 0 && (
            <p className="text-[8px] font-bold uppercase tracking-widest text-[#1A1A1A]/30 mt-4 mb-2">Papel</p>
          )}
