...
function PaperStatsBar({ items }: { items: PaperCatalogItem[] }) {
  const active = items.filter(i => i.enabled).length
  const lowStock = items.filter(i => {
    const curr = i.current_stock_m2 ?? 0
    const min = i.min_stock_m2 ?? 0
    return curr > 0 && min > 0 && curr < min
  }).length
  const noStock = items.filter(i => (i.current_stock_m2 ?? 0) === 0 && i.enabled).length
  const totalM2 = items.reduce((sum, i) => sum + (i.current_stock_m2 ?? 0), 0)
  ...
  const stats = [
    { icon: Activity,       label: 'Papeles activos',    value: active,                                                              accent: null },
    { icon: AlertTriangle,  label: 'Bajo stock',          value: lowStock,                                                            accent: lowStock > 0 ? 'amber' : null },
    { icon: PackageX,       label: 'Sin stock',           value: noStock,                                                             accent: noStock > 0 ? 'red' : null },
    { icon: Layers,         label: 'Stock total',         value: `${totalM2.toLocaleString('es', { maximumFractionDigits: 1 })} m²`,  accent: null },
    { icon: Clock,          label: 'Último movimiento',   value: lastUpdated ? relativeTime(lastUpdated) : '—',                       accent: null },
  ] as const
              ...
              .map((n, i) =>
                n === '…' ? (
                  ...
                  <button
                    ...
                    className={`size-7 text-[10px] font-bold border transition-colors ${safePage === n ? 'bg-foreground text-background border-foreground' : 'border-border hover:bg-muted'}`}
                  >
                    {n}
                  </button>
                )
